...
by the pinned `vendor/sshfighter` submodule as the mechanics reference separately
and explicitly records that the deployed commit is not attested. This repository
owns the runner, policy, configuration, and compatibility tests; the vendored
server remains a pinned protocol/mechanics dependency, not agent-owned runtime
code.
