...
import { type MatchPhase } from '../../vendor/sshfighter/src/game/types.js';
...
export function computeRunnerImplementationHash(): string {
  const ownerRepositoryRoot = fileURLToPath(new URL('../../', import.meta.url));
  const digest = createHash('sha256');
  for (const relative of RUNNER_IMPLEMENTATION_FILES) {
    digest.update(relative).update('\0');
    digest.update(readFileSync(resolve(ownerRepositoryRoot, relative))).update('\0');
  }
...
export function computeGoldenTraceHash(): string {
  const ownerRepositoryRoot = fileURLToPath(new URL('../../', import.meta.url));
  return sha256(readFileSync(resolve(ownerRepositoryRoot, 'src/fixtures/xenon-matchup-golden-trace.json')));
}

export function computePolicySourceFileHash(relative: string): string {
  const ownerRepositoryRoot = fileURLToPath(new URL('../../', import.meta.url));
  return sha256(readFileSync(resolve(ownerRepositoryRoot, 'src', relative)));
}
...
export function computeLegacyRuntimeSourceHash(): string {
  ...
  for (const relative of ['policies/xenon-legacy-runtime.ts']) {
    ...
    digest.update(readFileSync(resolve(ownerRepositoryRoot, 'src', relative))).update('\0');
  }
  return digest.digest('hex');
}
...
export function validateRunnerProvenance(actualHash = computeRunnerImplementationHash()): string {
  ...
  if (RUNNER_SOURCE_BASE_COMMIT !== '22839b24ec0d01777a9835c6272ccea0d92fcf31'
      || TARGET_DEPLOYMENT_PROFILE !== 'sshfighter-3caedf-unclose-17'
      || TARGET_ENGINE_COMMIT !== '3caedf3435c12996cf4d34fb5ac76c7cd7b75076')
...
export function runnerManifest(options: RunnerOptions): JsonObject {
  ...
  return {
    ...
    ownerRepositoryLayout: 'agent-roster-with-pinned-sshfighter-submodule',
    ...
    vendorSshfighterCommit: TARGET_ENGINE_COMMIT,
